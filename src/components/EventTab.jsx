...
import { CREATEVENTTYPES, DELETEVENTTYPE, EVENTTYPE, GETEVENTTYPE, UPDATEVENTTYPES } from '../pages/Helpers/url';
...
export default function EventTab() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await axios.get(GETEVENTTYPE, {
        headers: {
          "Authorization": `Bearer ${JSON.parse(token)}`
        }
      });

      setEvents(response.data.Eventtype);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!editItem) {
        // Update event
        const url = `${EVENTTYPE}eventtype/${editItem._id}`;
  ...
  const handleAddEventType = async (e) => {
    ...
    try {
      const data = {
        typeCode: 1, // Example typeCode value, adjust as per your API requirement 
        name: newEventType
      };
