...
import { CREATEVENTTYPES, DELETEVENTTYPE, EVENTTYPE, GETVENUE, UPDATEVENTTYPES } from '../pages/Helpers/url';

export default function EventVenu() {
...
const fetchEvents = async () => {
  try {
    ...
    const response = await axios.get(GETVENUE, {
      headers: {
        "Authorization": `Bearer ${JSON.parse(token)}`
      }
    });

    setEvents(response.data.venuetype);
  } catch (error) {
...
const handleAddEventType = async (e) => {
  ...
  try {
    const data = {
      typeCode: 2, // Example typeCode value, adjust as per your API requirement 
      name: newEventType
    };
